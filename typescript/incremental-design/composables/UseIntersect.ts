...
const useIntersectWithoutScroll: (...args:shared) => Ref<boolean>
= (el, container) => {
    /* detect intersect */
    const intersecting = ref(false);

    if (typeof window === "undefined") return intersecting; /* don't run in SSR */
...
const useIntersectWithScroll: (...args:[...shared:shared, scroll: Ref<boolean>]) => Ref<IsIntersecting>
= (el, container, scroll) => {
    const intersecting = ref(false);
    ...
    if (typeof window === "undefined") return ref(isIntersecting); /* don't run in SSR */
...
export type UseIntersect = (...args:[...shared:shared, scroll?: Ref<boolean>]) =>  typeof args[2] extends Ref<boolean> ? Ref<IsIntersecting> : Ref<boolean> /* read: if you DON'T supply scroll, you will only get a true/false back */
...
const u: UseIntersect = (el, container, scroll) => {
    return (typeof scroll === "undefined" ? useIntersectWithoutScroll(el, container) : useIntersectWithScroll(el, container, scroll)) as ReturnType<UseIntersect> // smh we need to assert the type or typescript will complain
}
